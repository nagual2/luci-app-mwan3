...
return view.extend({
	...
	handleAction: function(ev) {
		...
		switch (task.value) {
			case 'sync-track-routes':
				return this.handleCommand('/usr/sbin/mwan3',
					[ 'sync-track-routes' ]);
			case 'flush-conntrack':
				return this.handleCommand('/usr/sbin/mwan3',
					[ 'flush-conntrack' ]);
			case 'gateway':
	...
	render: function () {
		...
		var taskSel = [
			E('option', { 'value': 'sync-track-routes' }, [ _('Sync track host routes (nagual2)') ]),
			E('option', { 'value': 'flush-conntrack' }, [ _('Flush conntrack (nagual2)') ]),
